...
var game = {
	...
	reset: function() {
		...
		gameFrame.innerHTML = '<div id="explosionBox"><div id="explosionFrame"></div></div><div class="messageFrame"></div>';
	...
	start: function() {
		...
		document.querySelector("html").addEventListener("keydown", game.keydownhandler, true);
		document.querySelector("html").addEventListener("keyup", game.keyuphandler, true);
		document.querySelector("html").addEventListener("keyhold", game.keyholdhandler, true);
	...
	keyholdhandler: function(event) {
		...
		for (t in game.player2.tanks) {
			...
			switch (kC) {
				case 38:
					tank.v = game.player2.v/game.fps;
		...
		for (t in game.player1.tanks) {
			...
			switch (kC) {
				case 87:
					tank.v = game.player1.v/game.fps;
					if (!game.pauseDisabled && !game.paused) {
						tn1.loopAudio(100);
					}
					break;
	...
	explosion: function(x,y,tank1,tank2) {
		es.currentTime = 0;
		es.play();
		var frame = 0;
		...
		var t1 = document.querySelector("#tankA" + tank1) || false;
		var t2 = document.querySelector("#tankB" + tank2) || false;
	...
	keydownhandler: function(e) {
		...
		if (!game.keys[kC]) {
			game.keys[kC] = true;
			document.querySelector("html").dispatchEvent(new CustomEvent("keyhold", { detail: kC }));
		}
		//console.log(kC);
	}
}
